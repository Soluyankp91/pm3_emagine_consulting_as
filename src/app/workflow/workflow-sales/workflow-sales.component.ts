...
export class WorkflowSalesComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
            ...
            .pipe(
                ...
                switchMap((value: any) => {
                    if (value) {
                        ...
                        return this._lookupService.clients(
                            toSend.name,
            ...
            .subscribe((list: ClientResultDto[]) => {
                if (list.length) {
                    ...
                    this.filteredClientInvoicingRecipients = [
                        { clientName: 'No records found', id: 'no-data' },
                    ];
                }
            });
            ...
            .pipe(
                ...
                switchMap((value: any) => {
                    if (value) {
                        let toSend = {
                            clientId:
                                this.salesClientDataForm.directClientIdValue
                                    ?.value?.clientId,
                            name: value,
                            maxRecordsCount: 1000,
                        };
        ...
        merge(
            this.salesClientDataForm.clientContractStartDate!.valueChanges,
            ...
            .subscribe(() => {
                for (let consultant of this.consultantData.controls) {
                    if (
                        consultant.get('consultantProjectDurationSameAsClient')!
                            .value
                    ) {
                        consultant
                            .get('consultantProjectStartDate')
                            ?.setValue(
                                this.salesClientDataForm.clientContractStartDate
                                    ?.value,
                                { emitEvent: false }
                            );
                        consultant
                            ...
                            ?.setValue(
                                this.salesClientDataForm.clientContractEndDate
                                    ?.value,
                                { emitEvent: false }
                            );
                        consultant
                            ...
                            ?.setValue(
                                this.salesClientDataForm.clientContractNoEndDate
                                    ?.value,
                                { emitEvent: false }
                            );
                        if (
    ...
    ngOnInit(): void {
        ...
        this.filteredClientSpecialRates =
            this.clientSpecialRateFilter.valueChanges.pipe(
                map((value) => {
        ...
        this.filteredClientSpecialFees =
            this.clientSpecialFeeFilter.valueChanges.pipe(
                map((value) => {
                    if (typeof value === 'string') {
        ...
        this._workflowDataService.consultantStartChangeOrExtendSalesSaved
            ...
            .subscribe((isDraft: boolean) => {
                if (isDraft) {
                    ...
                    if (this.validateSalesForm()) {
                        this.saveStartChangeOrExtendConsultantPeriodSales(
                            isDraft
                        );
                    } else {
                        this.scrollToFirstError();
        ...
        this._workflowDataService.workflowSideSectionChanged
            ...
            .subscribe(
                (value: {
                    consultant?: ConsultantResultDto | undefined;
                    consultantPeriodId?: string | undefined;
                }) => {
                    this.getSalesStepData(
                        value?.consultant,
                        value?.consultantPeriodId
                    );
                }
            );
    ...
    private _filterClientRates(value: string): ClientSpecialRateDto[] {
        ...
        const selectedIds: number[] =
            this.salesClientDataForm.clientRates.value.map((x: any) => x.id);
    ...
    private _filterClientFees(value: string): ClientSpecialFeeDto[] {
        const filterValue = value.toLowerCase();
        const selectedIds: number[] =
            this.salesClientDataForm.clientFees.value.map((x: any) => x.id);
        const result = this.clientSpecialFeeList
            .filter((option) =>
                option.internalName!.toLowerCase().includes(filterValue)
            )
            .filter((x) => !selectedIds.includes(x.id!));
    ...
    getRatesAndFees(clientId: number) {
        this._clientService
            ...
            .subscribe(
                (result) =>
                    (this.clientSpecialRateList = result.filter(
                        (x) => !x.isHidden
                    ))
            );
        this._clientService
            ...
            .subscribe(
                (result) =>
                    (this.clientSpecialFeeList = result.filter(
                        (x) => !x.isHidden
                    ))
            );
    }
    ...
    getInvoicingTimes() {
        this._internalLookupService
            .getInvoicingTimes()
            .subscribe((result) => (this.invoicingTimes = result));
    }

    getInvoiceFrequencies() {
        this._internalLookupService
            .getInvoiceFrequencies()
            .subscribe((result) => (this.invoiceFrequencies = result));
    }

    getSignerRoles() {
        this._internalLookupService
            .getSignerRoles()
            .subscribe((result) => (this.signerRoles = result));
    }

    getEmagineOfficeList() {
        this._internalLookupService
            .getEmagineOfficeList()
            .subscribe((result) => (this.emagineOffices = result));
    }
    ...
    getExtensionDeadlines() {
        this._internalLookupService
            .getExtensionDeadlines()
            .subscribe((result) => (this.clientExtensionDeadlines = result));
    }

    getExtensionDurations() {
        this._internalLookupService
            .getExtensionDurations()
            .subscribe((result) => (this.clientExtensionDurations = result));
    }
    ...
    getSpecialFeeSpecifications() {
        this._internalLookupService
            .getSpecialFeeSpecifications()
            .subscribe(
                (result) => (this.clientSpecialFeeSpecifications = result)
            );
    }
    ...
    getSpecialRateSpecifications() {
        this._internalLookupService
            .getSpecialRateSpecifications()
            .subscribe(
                (result) => (this.clientSpecialRateSpecifications = result)
            );
    }
    ...
    getCommissionFrequency() {
        this._internalLookupService
            .getCommissionFrequency()
            .subscribe((result) => (this.commissionFrequencies = result));
    }

    getCommissionTypes() {
        this._internalLookupService
            .getCommissionTypes()
            .subscribe((result) => (this.commissionTypes = result));
    }
    ...
    getProjectCategory() {
        this._internalLookupService
            .getProjectCategory()
            .subscribe((result) => (this.projectCategories = result));
    }
    ...
    getTerminationTimes() {
        this._internalLookupService
            .getTerminationTimes()
            .subscribe((result) => (this.nonStandartTerminationTimes = result));
    }

    getTerminationReasons() {
        this._internalLookupService
            .getTerminationReasons()
            .subscribe((result) => (this.terminationReasons = result));
    }

    getExpectedWorkloadUnit() {
        this._internalLookupService
            .getExpectedWorkloadUnit()
            .subscribe((result) => (this.expectedWorkloadUnits = result));
    }
    ...
    getDataBasedOnProjectType(event: MatSelectChange) {
        ...
        this._clientPeriodService
            ...
            .subscribe((result) => {
                this.salesMainDataForm.deliveryType?.setValue(
                    this.findItemById(
                        this.deliveryTypes,
                ...
                this.salesMainDataForm.salesType?.setValue(
                    this.findItemById(this.saleTypes, result.salesTypeId),
                    { emitEvent: false }
                );
                this.salesMainDataForm.margin?.setValue(
                    this.findItemById(this.margins, result.marginId),
                    { emitEvent: false }
                );
            });
    }

    selectClientSpecialRate(
        event: any,
        ...
        if (formattedRate.rateSpecifiedAs?.id === 1) {
            formattedRate.clientRate = +(
                (this.salesClientDataForm?.clientPrice?.value *
                    rate.clientRate!) /
                100
            ).toFixed(2);
            formattedRate.clientRateCurrencyId =
                this.salesClientDataForm.clientCurrency?.value?.id;
        } else {
    ...
    addSpecialRate(clientRate?: PeriodClientSpecialRateDto) {
        const form = this._fb.group({
            ...
            rateSpecifiedAs: new UntypedFormControl(
                clientRate?.rateSpecifiedAs ?? null
            ),
    ...
    addSignerToForm(signer?: ContractSignerDto) {
        const form = this._fb.group({
            ...
            clientRole: new UntypedFormControl(
                this.findItemById(this.signerRoles, signer?.signerRoleId) ??
                    null
            ),
            clientSequence: new UntypedFormControl(signer?.signOrder ?? null),
        });
        this.salesClientDataForm.contractSigners.push(form);
        this.manageSignersContactAutocomplete(
            this.salesClientDataForm.contractSigners.length - 1
        );
    }

    manageSignersContactAutocomplete(signerIndex: number) {
        let arrayControl =
            this.salesClientDataForm.contractSigners.at(signerIndex);
        arrayControl!
            .get('clientContact')!
            .valueChanges.pipe(
                takeUntil(this._unsubscribe),
    ...
    get contractSigners(): UntypedFormArray {
        return this.salesClientDataForm.get(
            'contractSigners'
        ) as UntypedFormArray;
    }
    ...
    addConsultantForm(consultant?: ConsultantSalesDataDto) {
        ...
        consultantDto.sourcingRequestConsultantId =
            consultant?.soldRequestConsultantId;
        ...
        const form = this._fb.group({
            ...
            consultantName: new UntypedFormControl(consultantDto ?? null),
            // requestConsultantId: new FormControl(consultant?.soldRequestConsultantId ?? null),
            consultantPeriodId: new UntypedFormControl(
                consultant?.consultantPeriodId ?? null
            ),
            consultantNameOnly: new UntypedFormControl(
                consultant?.nameOnly ?? null
            ),

            consultantProjectDurationSameAsClient: new UntypedFormControl(
                consultant?.durationSameAsClientPeriod ?? true
            ),
            consultantProjectStartDate: new UntypedFormControl(
                consultant?.startDate ?? null
            ),
            consultantProjectEndDate: new UntypedFormControl({
                value: consultant?.endDate ?? null,
                disabled: consultant?.noEndDate,
            }),
            consultantProjectNoEndDate: new UntypedFormControl(
                consultant?.noEndDate ?? false
            ),

            consultantWorkplace: new UntypedFormControl(null),
            consultantWorkplaceClientAddress: new UntypedFormControl(
                consultant?.onsiteClient ?? null
            ),
            consultantWorkplaceEmagineOffice: new UntypedFormControl(
                this.findItemById(
                    this.emagineOffices,
                    consultant?.emagineOfficeId
                ) ?? null
            ),
            consultantWorkplaceRemote: new UntypedFormControl(
                this.findItemById(
                    this.countries,
                    consultant?.remoteAddressCountryId
                ) ?? null
            ),
            consultantWorkplacePercentageOnSite: new UntypedFormControl(
                consultant?.percentageOnSite ?? null,
                [Validators.min(1), Validators.max(100)]
            ),

            consultantIsOnsiteWorkplace: new UntypedFormControl(
                consultant?.isOnsiteWorkplace ?? false
            ),
            consultantIsEmagineOfficeWorkplace: new UntypedFormControl(
                consultant?.isEmagineOfficeWorkplace ?? false
            ),
            consultantIsRemoteWorkplace: new UntypedFormControl(
                consultant?.isRemoteWorkplace ?? false
            ),

            expectedWorkloadHours: new UntypedFormControl(
                consultant?.expectedWorkloadHours ?? null
            ),
            expectedWorkloadUnitId: new UntypedFormControl(
                this.findItemById(
                    this.expectedWorkloadUnits,
                    consultant?.expectedWorkloadUnitId
                ) ?? null
            ),
            consultantCapOnTimeReporting: new UntypedFormControl(
                this.findItemById(
                    this.consultantTimeReportingCapList,
                    consultant?.consultantTimeReportingCapId ?? 4
                )
            ), // ?? default value = no cap - id:4
            consultantTimeReportingCapMaxValue: new UntypedFormControl(
                consultant?.consultantTimeReportingCapMaxValue ?? null
            ),
            ...
            consultantRate: new UntypedFormControl(
                consultant?.consultantRate?.normalRate ?? null
            ),
            ...
            consultantPDCRate: new UntypedFormControl(
                consultant?.consultantRate?.prodataToProdataRate ?? null
            ),
            ...
            consultantPDCRateCurrency: new UntypedFormControl(
                this.findItemById(
                    this.currencies,
                    consultant?.consultantRate?.prodataToProdataCurrencyId
                ) ?? null
            ),
            ...
            consultantInvoicingFrequency: new UntypedFormControl(
                this.findItemById(
                    ...
                    consultant?.consultantRate?.invoiceFrequencyId
                ) ?? null
            ),
            consultantInvoicingTime: new UntypedFormControl(
                this.findItemById(
                    this.invoicingTimes,
                    consultant?.consultantRate?.invoicingTimeId
                ) ?? null
            ),
            consultantInvoicingManualDate: new UntypedFormControl(
                consultant?.consultantRate?.manualDate ?? null
            ),
            prodataToProdataInvoiceCurrency: new UntypedFormControl(
                this.findItemById(
                    this.currencies,
                    consultant?.consultantRate
                        ?.prodataToProdataInvoiceCurrencyId
                ) ?? null
            ),
            ...
            consultantSpecialContractTermsNone: new UntypedFormControl(
                consultant?.noSpecialContractTerms ?? false
            ),
            ...
            deliveryManagerSameAsAccountManager: new UntypedFormControl(
                consultant?.deliveryManagerSameAsAccountManager ?? false
            ),
        ...
        if (consultant?.periodConsultantSpecialRates?.length) {
            for (let rate of consultant?.periodConsultantSpecialRates) {
                this.addConsultantSpecialRate(
                    this.consultantsForm.consultantData.length - 1,
                    rate
                );
            }
        }
        if (consultant?.periodConsultantSpecialFees?.length) {
            for (let fee of consultant?.periodConsultantSpecialFees) {
                this.addConsultantSpecialFee(
                    this.consultantsForm.consultantData.length - 1,
                    fee
                );
            }
        }
        this.manageManagerAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
        this.manageConsultantAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
        this.manageConsultantRateAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
        this.manageConsultantFeeAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
        this.manageConsultantClientAddressAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
        this.manageConsultantCountryAutocomplete(
            this.consultantsForm.consultantData.length - 1
        );
    }
    ...
    updateConsultantStepAnchors() {
        let consultantNames = this.consultantData.value.map((item: any) => {
            if (
                item.employmentType?.id === 10 ||
                item.employmentType?.id === 11
            ) {
    ...
    manageManagerAutocomplete(consultantIndex: number) {
        let arrayControl =
            this.consultantsForm.consultantData.at(consultantIndex);
        arrayControl!
    ...
    manageConsultantCountryAutocomplete(consultantIndex: number) {
        ...
        arrayControl!
            ...
            .subscribe((value) => {
                if (typeof value === 'string') {
                    this.filteredConsultantCountries =
                        this._filterConsultantCountry(value);
                }
            });
    }
    ...
    private _filterConsultantCountry(value: string): CountryDto[] {
        const filterValue = value.toLowerCase();
        const result = this.countries.filter((option) =>
    ...
    updateProdataUnitType(event: MatSelectChange, consultantIndex: number) {
        this.consultantData
            .at(consultantIndex)
            .get('consultantPDCRateUnitType')
            ?.setValue(event.value, { emitEvent: false });
    }

    getConsultantRateControls(
        consultantIndex: number
    ): AbstractControl[] | null {
        return (
            this.consultantsForm.consultantData
                .at(consultantIndex)
    ...
    removeConsutlantRate(consultantIndex: number, specialRateIndex: number) {
        (
            this.consultantsForm.consultantData
                .at(consultantIndex)
                .get('specialRates') as UntypedFormArray
        ).removeAt(specialRateIndex);
    }

    editOrSaveConsultantRate(
        consultantIndex: number,
        ...
        if (isEditable) {
            ...
            this.consultantRateToEdit = new PeriodConsultantSpecialRateDto({
                ...
                prodataToProdataRateCurrencyId:
                    consultantRateValue.prodataToProdataRateCurrency?.id,
    ...
    addConsultantSpecialRate(
        consultantIndex: NumberSymbol,
        consultantRate?: PeriodConsultantSpecialRateDto
    ) {
        const form = this._fb.group({
            id: new UntypedFormControl(consultantRate?.id ?? null),
            clientSpecialRateId: new UntypedFormControl(
                consultantRate?.clientSpecialRateId ?? null
            ),
            rateName: new UntypedFormControl(consultantRate?.rateName ?? null),
            reportingUnit: new UntypedFormControl(
                consultantRate?.reportingUnit ?? null
            ),
            prodataToProdataRate: new UntypedFormControl(
                consultantRate?.prodataToProdataRate ?? null
            ),
    ...
    getConsultantFeeControls(
        consultantIndex: number
    ): AbstractControl[] | null {
        return (
            this.consultantsForm.consultantData
                .at(consultantIndex)
                .get('specialFees') as UntypedFormArray
        ).controls;
    }
    ...
    removeConsutlantFee(consultantIndex: number, specialFeeIndex: number) {
        (
            this.consultantsForm.consultantData
                .at(consultantIndex)
                .get('specialFees') as UntypedFormArray
        ).removeAt(specialFeeIndex);
    }

    editOrSaveConsultantFee(
        consultantIndex: number,
        specialFeeIndex: number,
        isEditable: boolean
    ) {
        if (isEditable) {
            // save
            ...
            this.consultantFeeToEdit = new PeriodConsultantSpecialFeeDto({
                ...
                prodataToProdataRateCurrencyId:
                    consultantFeeValue.prodataToProdataRateCurrency?.id,
    ...
    addConsultantSpecialFee(
        ...
        const form = this._fb.group({
            ...
            consultantRate: new UntypedFormControl(
                consultantFee?.consultantRate ?? null
            ),
            consultantRateCurrency: new UntypedFormControl(
                this.findItemById(
                    this.currencies,
            ...
            .subscribe((list: ConsultantWithSourcingRequestResultDto[]) => {
                if (list.length) {
                    ...
                    this.filteredConsultants = [
                        {
                            consultant: { name: 'No consultant found' },
                            externalId: '',
                            id: 'no-data',
                        },
                    ];
    ...
    confirmRemoveConsultant(index: number) {
        ...
        const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
            ...
            data: {
                confirmationMessageTitle: `Are you sure you want to delete consultant ${
    ...
    get consultantData(): UntypedFormArray {
        return this.consultantsForm.get('consultantData') as UntypedFormArray;
    }
    ...
    saveStartChangeOrExtendClientPeriodSales(isDraft: boolean) {
        ...
        input.salesMainData.salesTypeId =
            this.salesMainDataForm.salesType?.value?.id;
        input.salesMainData.deliveryTypeId =
            this.salesMainDataForm.deliveryType?.value?.id;
        input.salesMainData.marginId = this.salesMainDataForm.margin?.value?.id;
        input.salesMainData.projectCategoryId =
            this.salesMainDataForm.projectCategory?.value?.id;
        input.salesMainData.projectDescription =
            this.salesMainDataForm.projectDescription?.value;
        input.salesMainData.projectName =
            this.salesMainDataForm.projectName?.value;

        input.salesMainData.discountId =
            this.salesMainDataForm.discounts?.value?.id;
        input.salesMainData.salesAccountManagerIdValue =
            this.salesMainDataForm.salesAccountManagerIdValue?.value?.id;
        input.salesMainData.commissionAccountManagerIdValue =
            this.salesMainDataForm.commissionAccountManagerIdValue?.value?.id;
        input.salesMainData.customContractExpirationNotificationDate =
            this.salesMainDataForm.contractExpirationNotification?.value?.includes(
                999
            )
                ? this.salesMainDataForm
                      .customContractExpirationNotificationDate?.value
                : null;
        input.salesMainData.contractExpirationNotificationIntervalIds =
            this.salesMainDataForm.contractExpirationNotification?.value?.filter(
                (x: number) => x !== 999
            );
        ...
        if (this.salesMainDataForm.commissions.value?.length) {
            this.salesMainDataForm.commissions.value.forEach(
                (commission: any) => {
                    ...
                    commissionInput.commissionFrequencyId =
                        commission.frequency?.id;
                    commissionInput.recipientTypeId =
                        commission.recipientType?.id;
                    switch (commission.recipientType?.id) {
                        case 1: // Supplier
                            commissionInput.supplierId =
                                commission.recipient?.supplierId;
                            break;
                        case 2: //Consultant
                            commissionInput.consultantId =
                                commission.recipient?.id;
                            break;
                        case 3: // client
                            commissionInput.clientId =
                                commission.recipient?.clientId;
                            break;
                        case 4: // PDC entity
        ...
        input.salesClientData.endClientIdValue =
            this.salesClientDataForm.endClientIdValue?.value?.clientId;
        input.startDate =
            this.salesClientDataForm.clientContractStartDate?.value;
        input.noEndDate =
            this.salesClientDataForm.clientContractNoEndDate?.value;
        input.endDate = this.salesClientDataForm.clientContractEndDate?.value;
        input.salesClientData.noClientExtensionOption =
            this.salesClientDataForm.noClientExtensionOption?.value;
        input.salesClientData.clientExtensionDurationId =
            this.salesClientDataForm.clientExtensionDuration?.value?.id;
        input.salesClientData.clientExtensionDeadlineId =
            this.salesClientDataForm.clientExtensionDeadline?.value?.id;
        input.salesClientData.clientExtensionSpecificDate =
            this.salesClientDataForm.clientExtensionEndDate?.value;
        input.salesClientData.clientTimeReportingCapId =
            this.salesClientDataForm.capOnTimeReporting?.value?.id;
        input.salesClientData.clientTimeReportingCapMaxValue =
            this.salesClientDataForm.capOnTimeReportingValue?.value;
        input.salesClientData.pdcInvoicingEntityId =
            this.salesClientDataForm.pdcInvoicingEntityId?.value?.id;

        input.salesClientData.clientRate = new ClientRateDto();
        input.salesClientData.clientRate.isTimeBasedRate =
            this.salesClientDataForm.clientRateAndInvoicing?.value?.id === 1; // 1: 'Time based';
        input.salesClientData.clientRate.isFixedRate =
            this.salesClientDataForm.clientRateAndInvoicing?.value?.id === 2; // 2: 'Fixed';
        input.salesClientData.clientRate.currencyId =
            this.salesClientDataForm.clientCurrency?.value?.id;
        input.salesClientData.clientRate.invoiceCurrencyId =
            this.salesClientDataForm.clientInvoiceCurrency?.value?.id;
        input.salesClientData.clientRate.normalRate =
            this.salesClientDataForm.clientPrice?.value;
        input.salesClientData.clientRate.rateUnitTypeId =
            this.salesClientDataForm.rateUnitTypeId?.value?.id;
        input.salesClientData.clientRate.invoiceFrequencyId =
            this.salesClientDataForm.clientInvoiceFrequency?.value?.id;
        input.salesClientData.clientRate.invoicingTimeId =
            this.salesClientDataForm.clientInvoiceTime?.value?.id;
        input.salesClientData.clientRate.manualDate =
            this.salesClientDataForm.clientInvoicingDate?.value;

        input.salesClientData.noInvoicingReferenceNumber = this
            .salesClientDataForm.invoicingReferenceNumber?.value
            ? false
            : true;
        input.salesClientData.invoicingReferenceNumber =
            this.salesClientDataForm.invoicingReferenceNumber?.value;
        input.salesClientData.clientInvoicingRecipientSameAsDirectClient =
            this.salesClientDataForm.clientInvoicingRecipientSameAsDirectClient?.value;
        ...
        input.salesClientData.noInvoicingReferencePerson =
            this.salesClientDataForm.noInvoicingReferencePerson?.value;
        input.salesClientData.invoicingReferencePersonIdValue =
            this.salesClientDataForm.invoicingReferencePersonIdValue?.value?.id;

        if (this.salesClientDataForm.clientRates.value.length) {
            input.salesClientData.periodClientSpecialRates =
            ...
            this.salesClientDataForm.clientRates.value.forEach((rate: any) => {
                ...
                input.salesClientData!.periodClientSpecialRates?.push(
                    clientRate
                );
            });
        } else {
        ...
        if (this.salesClientDataForm.clientFees.value.length) {
            input.salesClientData.periodClientSpecialFees =
        ...
        input.salesClientData.evaluationsDisabled =
            this.salesClientDataForm.evaluationsDisabled?.value;
        input.salesClientData.evaluationsDisabledReason =
            this.salesClientDataForm.evaluationsDisabledReason?.value;
        input.salesClientData.noSpecialContractTerms =
            this.salesClientDataForm.noSpecialContractTerms?.value;
        input.salesClientData.specialContractTerms =
            this.salesClientDataForm.specialContractTerms?.value;
        ...
        if (this.salesClientDataForm.contractSigners.value?.length) {
            this.salesClientDataForm.contractSigners.value.forEach(
                (signer: any) => {
        ...
        if (this.consultantsForm.consultantData.value?.length) {
            this.consultantsForm.consultantData.value.forEach(
                (consultant: any) => {
                    let consultantInput = new ConsultantSalesDataDto();
                    consultantInput.employmentTypeId =
                        consultant.employmentType?.id;
                    //  IF employment type is 'Fee only' - id: 10 or 'Recruitment' - id:11 only name is stored
                    if (
                        consultant.employmentType?.id === 10 ||
                        consultant.employmentType?.id === 11
                    ) {
                        consultantInput.nameOnly =
                            consultant.consultantNameOnly;
                    } else {
                        consultantInput.consultantId =
                            consultant.consultantName?.consultant?.id;
                        consultantInput.soldRequestConsultantId =
                            consultant.consultantName?.sourcingRequestConsultantId;
                        consultantInput.consultantPeriodId =
                            consultant.consultantPeriodId;
                        consultantInput.consultant = new ConsultantResultDto();
                        consultantInput.consultant.id =
                            consultant.consultantName?.consultant?.id;
                        consultantInput.consultant.name =
                            consultant.consultantName?.consultant?.name;
                        consultantInput.consultant.legacyId =
                            consultant.consultantName?.consultant?.legacyId;
                        consultantInput.consultant.companyName =
                            consultant.consultantName?.consultant?.companyName;
                        consultantInput.consultant.tenantId =
                            consultant.consultantName?.consultant?.tenantId;
                        consultantInput.consultant.externalId =
                            consultant.consultantName?.consultant?.externalId;
                        consultantInput.consultant.city =
                            consultant.consultantName?.consultant?.city;
                        consultantInput.consultant.countryId =
                            consultant.consultantName?.consultant?.contryId;
                        ...
                        consultantInput.consultantRate =
                            new ConsultantRateDto();
                        consultantInput.consultantRate.isTimeBasedRate =
                            consultant.consultantPaymentType?.id === 1; // 1: 'Time based';
                        consultantInput.consultantRate.isFixedRate =
                            consultant.consultantPaymentType?.id === 2; // 2: 'Fixed';
                        consultantInput.consultantRate.normalRate =
                            consultant.consultantRate;
                        consultantInput.consultantRate.currencyId =
                            consultant.consultantRateCurrency?.id;
                        consultantInput.consultantRate.rateUnitTypeId =
                            consultant.consultantRateUnitType?.id;
                        consultantInput.consultantRate.prodataToProdataRate =
                            consultant.consultantPDCRate;
                        consultantInput.consultantRate.prodataToProdataCurrencyId =
                            consultant.consultantPDCRateCurrency?.id;
                        consultantInput.consultantRate.prodataToProdataInvoiceCurrencyId =
                            consultant.prodataToProdataInvoiceCurrency?.id;
                        if (consultantInput.consultantRate.isTimeBasedRate) {
                            consultantInput.consultantRate.invoiceFrequencyId =
                                consultant.consultantInvoicingFrequency?.id;
                        }
                        if (consultantInput.consultantRate.isFixedRate) {
                            consultantInput.consultantRate.invoicingTimeId =
                                consultant.consultantInvoicingTime?.id;
                        }
                        consultantInput.consultantRate.manualDate =
                            consultant.manualDate;

                        if (consultant.specialRates.length) {
                            consultantInput.periodConsultantSpecialRates =
                                new Array<PeriodConsultantSpecialRateDto>();
                            for (let rate of consultant.specialRates) {
                                const consultantSpecialRate =
                                    new PeriodConsultantSpecialRateDto();
                                consultantSpecialRate.id = rate.id;
                                consultantSpecialRate.clientSpecialRateId =
                                    rate.clientSpecialRateId;
                                consultantSpecialRate.rateName = rate.rateName;
                                consultantSpecialRate.reportingUnit =
                                    rate.reportingUnit;
                                consultantSpecialRate.prodataToProdataRate =
                                    rate.prodataToProdataRate;
                                consultantSpecialRate.prodataToProdataRateCurrencyId =
                                    rate.prodataToProdataRateCurrency?.id;
                                consultantSpecialRate.consultantRate =
                                    rate.consultantRate;
                                consultantSpecialRate.consultantRateCurrencyId =
                                    rate.consultantRateCurrency?.id;
                                consultantInput.periodConsultantSpecialRates.push(
                                    consultantSpecialRate
                                );
                            }
                        } else {
                        ...
                        if (consultant.specialFees.length) {
                            consultantInput.periodConsultantSpecialFees =
                                new Array<PeriodConsultantSpecialFeeDto>();
                            for (let fee of consultant.specialFees) {
                                const consultantSpecialFee =
                                    new PeriodConsultantSpecialFeeDto();
                                consultantSpecialFee.id = fee.id;
                                consultantSpecialFee.clientSpecialFeeId =
                                    fee.clientSpecialFeeId;
                                consultantSpecialFee.feeName = fee.feeName;
                                consultantSpecialFee.frequency = fee.frequency;
                                consultantSpecialFee.prodataToProdataRate =
                                    fee.prodataToProdataRate;
                                consultantSpecialFee.prodataToProdataRateCurrencyId =
                                    fee.prodataToProdataRateCurrency?.id;
                                consultantSpecialFee.consultantRate =
                                    fee.consultantRate;
                                consultantSpecialFee.consultantRateCurrencyId =
                                    fee.consultantRateCurrency?.id;
                                consultantInput.periodConsultantSpecialFees.push(
                                    consultantSpecialFee
                                );
                            }
                        } else {
                        ...
                        consultantInput.specialContractTerms =
                            consultant.consultantSpecialContractTerms;
                        consultantInput.deliveryManagerSameAsAccountManager =
                            consultant.deliveryManagerSameAsAccountManager;
                        consultantInput.deliveryAccountManagerIdValue =
                            consultant.deliveryAccountManager?.id;
                    }
        ...
        this._clientPeriodService
            ...
            .subscribe((result) => {
                ...
                this.salesMainDataForm.projectType?.setValue(
                    this.findItemById(
                        this.projectTypes,
                        result?.salesMainData?.projectTypeId
                    ),
                    { emitEvent: false }
                );
                this.salesMainDataForm.salesType?.setValue(
                    this.findItemById(
                        this.saleTypes,
                        result?.salesMainData?.salesTypeId
                    ),
                    { emitEvent: false }
                );
                this.salesMainDataForm.deliveryType?.setValue(
                    this.findItemById(
                        this.deliveryTypes,
                        result?.salesMainData?.deliveryTypeId
                    ),
                    { emitEvent: false }
                );
                this.salesMainDataForm.projectCategory?.setValue(
                    this.findItemById(
                        this.projectCategories,
                        result?.salesMainData?.projectCategoryId
                    ),
                    { emitEvent: false }
                );
                this.salesMainDataForm.margin?.setValue(
                    this.findItemById(
                        this.margins,
                        result?.salesMainData?.marginId
                    ),
                    { emitEvent: false }
                );
                this.salesMainDataForm.projectDescription?.setValue(
                    result?.salesMainData?.projectDescription,
                    { emitEvent: false }
                );
                this.salesMainDataForm.projectName?.setValue(
                    result?.salesMainData?.projectName,
                    { emitEvent: false }
                );

                // Invoicing
                result.salesMainData?.commissions?.forEach(
                ...
                this.salesMainDataForm.discounts?.setValue(
                    this.findItemById(
                        this.discounts,
                        result?.salesMainData?.discountId ?? 1
                    ),
                    { emitEvent: false }
                ); // 1 - default value 'None'

                // Account Manager
                this.salesMainDataForm.salesAccountManagerIdValue?.setValue(
                ...
                let expirationNotificationIntervals =
                    result.salesMainData
                        ?.contractExpirationNotificationIntervalIds;
                if (
                    result?.salesMainData
                        ?.customContractExpirationNotificationDate !== null &&
                    result?.salesMainData
                        ?.customContractExpirationNotificationDate !== undefined
                ) {
                    expirationNotificationIntervals!.push(999);
                }
                this.contractExpirationNotificationDisplay =
                    this.formatExpirationNotificationsForDisplay(
                        expirationNotificationIntervals
                    );
                this.salesMainDataForm.contractExpirationNotification?.setValue(
                    expirationNotificationIntervals,
                    { emitEvent: false }
                );
                this.salesMainDataForm.customContractExpirationNotificationDate?.setValue(
                    result?.salesMainData
                        ?.customContractExpirationNotificationDate,
                    { emitEvent: false }
                );
                this.salesMainDataForm.remarks?.setValue(
                    result?.salesMainData?.remarks,
                    { emitEvent: false }
                );
                this.salesMainDataForm.noRemarks?.setValue(
                    result?.salesMainData?.noRemarks,
                    { emitEVent: false }
                );
                ...
                // Client
                this.salesClientDataForm.differentEndClient?.setValue(
                    result.salesClientData?.differentEndClient ?? false,
                    { emitEvent: false }
                ); // default value if false
                this.salesClientDataForm.directClientIdValue?.setValue(
                    result?.salesClientData?.directClient,
                    { emitEvent: false }
                );
                if (result?.salesClientData?.directClient?.clientId) {
                    this.getRatesAndFees(
                        result?.salesClientData?.directClient?.clientId
                    );
                }
                this.salesClientDataForm.endClientIdValue?.setValue(
                    result?.salesClientData?.endClient,
                    { emitEvent: false }
                );
                //Duration
                this.salesClientDataForm.clientContractStartDate?.setValue(
                    result?.startDate,
                    { emitEvent: false }
                );
                this.salesClientDataForm.clientContractEndDate?.setValue(
                    result?.endDate,
                    { emitEvent: false }
                );
                this.salesClientDataForm.clientContractNoEndDate?.setValue(
                    result?.noEndDate,
                    { emitEvent: false }
                );
                if (result?.noEndDate) {
                    this.salesClientDataForm.clientContractEndDate?.disable({
                        emitEvent: false,
                    });
                }
                ...
                this.salesClientDataForm.clientExtensionDuration?.setValue(
                    this.findItemById(
                        this.clientExtensionDurations,
                        result?.salesClientData?.clientExtensionDurationId
                    ),
                    { emitEvent: false }
                );
                this.salesClientDataForm.clientExtensionDeadline?.setValue(
                    this.findItemById(
                        this.clientExtensionDeadlines,
                        result?.salesClientData?.clientExtensionDeadlineId
                    ),
                    { emitEvent: false }
                );
                // Project
                this.salesClientDataForm.capOnTimeReporting?.setValue(
                    this.findItemById(
                ...
                this.salesClientDataForm.capOnTimeReportingValue?.setValue(
                ...
                this.salesClientDataForm.pdcInvoicingEntityId?.setValue(
                    this.findItemById(
                        this.tenants,
                        result?.salesClientData?.pdcInvoicingEntityId
                    ),
                    { emitEvent: false }
                );
                ...
                if (result.salesClientData?.clientRate?.isFixedRate) {
                    ...
                    result.salesClientData?.clientRate?.isTimeBasedRate
                ) {
                    clientRateType = this.findItemById(this.clientRateTypes, 1); // 1: 'Time based'
                }
                this.salesClientDataForm.clientRateAndInvoicing?.setValue(
                    clientRateType,
                    { emitEVent: false }
                );

                this.salesClientDataForm.clientPrice?.setValue(
                    result.salesClientData?.clientRate?.normalRate,
                    { emitEVent: false }
                );
                this.salesClientDataForm.rateUnitTypeId?.setValue(
                    this.findItemById(
                        this.rateUnitTypes,
                ...
                this.salesClientDataForm.clientCurrency?.setValue(
                    this.findItemById(
                        this.currencies,
                        result.salesClientData?.clientRate?.currencyId
                    ),
                    { emitEVent: false }
                );
                this.salesClientDataForm.clientInvoiceCurrency?.setValue(
                    this.findItemById(
                        this.currencies,
                ...
                if (
                    this.salesClientDataForm.clientRateAndInvoicing?.value
                        ?.id === 1
                ) {
                    // Time based
                    this.salesClientDataForm.clientInvoiceFrequency?.setValue(
                        this.findItemById(
                            this.invoiceFrequencies,
                            result.salesClientData?.clientRate
                                ?.invoiceFrequencyId
                        ),
                        { emitEVent: false }
                    );
                ...
                if (
                    ...
                    this.salesClientDataForm.clientInvoiceTime?.setValue(
                        this.findItemById(
                            this.invoicingTimes,
                            result.salesClientData?.clientRate?.invoicingTimeId
                        ),
                        { emitEVent: false }
                    );
                }
                this.salesClientDataForm.clientInvoicingDate?.setValue(
                    result.salesClientData?.clientRate?.manualDate,
                    { emitEVent: false }
                );

                this.salesClientDataForm.invoicingReferenceNumber?.setValue(
                    result.salesClientData?.invoicingReferenceNumber,
                    { emitEVent: false }
                );
                this.salesClientDataForm.clientInvoicingRecipientIdValue?.setValue(
                ...
                this.salesClientDataForm.clientInvoicingRecipientSameAsDirectClient?.setValue(
                    result?.salesClientData
                        ?.clientInvoicingRecipientSameAsDirectClient,
                    { emitEvent: false }
                );
                if (
                    result?.salesClientData
                ...
                this.salesClientDataForm.invoicingReferencePersonIdValue?.setValue(
                    result?.salesClientData?.invoicingReferencePerson,
                    { emitEvent: false }
                );
                this.salesClientDataForm.noInvoicingReferencePerson?.setValue(
                    result?.salesClientData?.noInvoicingReferencePerson,
                    { emitEvent: false }
                );
                if (result?.salesClientData?.noInvoicingReferencePerson) {
                    this.salesClientDataForm.invoicingReferencePersonIdValue?.disable(
                        { emitEvent: false }
                    );
                }

                // Rates & Fees
                result.salesClientData?.periodClientSpecialRates?.forEach(
                    (specialRate: PeriodClientSpecialRateDto) => {
                        this.addSpecialRate(specialRate);
                    }
                );

                result.salesClientData?.periodClientSpecialFees?.forEach(
                    (specialFee: PeriodClientSpecialFeeDto) => {
                ...
                this.salesClientDataForm.evaluationsDisabled?.setValue(
                    result?.salesClientData?.evaluationsDisabled ?? false,
                    { emitEvent: false }
                ); // enabled - defalut value
                this.salesClientDataForm.evaluationsDisabledReason?.setValue(
                    result?.salesClientData?.evaluationsDisabledReason,
                    { emitEvent: false }
                );
                // Contract
                this.salesClientDataForm.noSpecialContractTerms?.setValue(
                    result?.salesClientData?.noSpecialContractTerms,
                    { emitEvent: false }
                );
                this.salesClientDataForm.specialContractTerms?.setValue(
                    result?.salesClientData?.specialContractTerms,
                    { emitEvent: false }
                );
                if (result?.salesClientData?.noSpecialContractTerms) {
                    this.salesClientDataForm.specialContractTerms?.disable();
                }
                result?.salesClientData?.contractSigners?.forEach(
                    (signer: ContractSignerDto) => {
    ...
    clientRateTypeChange(value: EnumEntityTypeDto) {
        if (value) {
            this.salesClientDataForm.rateUnitTypeId?.setValue(null, {
                emitEvent: false,
            });
            this.salesClientDataForm.clientPrice?.setValue(null, {
                emitEvent: false,
            });
            this.salesClientDataForm.clientCurrency?.setValue(null, {
                emitEvent: false,
            });
        }
    }
    ...
    salesTypeChange(value: EnumEntityTypeDto) {
        if (value.id === 3) {
            ...
            this.salesMainDataForm.deliveryType?.setValue(itemToPreselct, {
                emitEvent: false,
            });
        }
    }

    changeConsultantWorkplace(
        event: MatCheckboxChange,
        consutlantIndex: number
    ) {
        if (event.checked) {
            this.consultantData
                .at(consutlantIndex)
    ...
    changeConsultantData(index: number) {
        const consultantData =
            this.consultantsForm.consultantData.at(index).value;
        const scrollStrategy = this.overlay.scrollStrategies.reposition();
        const dialogRef = this.dialog.open(
            WorkflowConsultantActionsDialogComponent,
            {
                ...
                data: {
                    ...
                    consultantData: {
                        externalId:
                            consultantData.consultantName.consultant.externalId,
    ...
    addCommission(isInitial?: boolean, commission?: CommissionDto) {
        ...
        switch (commission?.recipientTypeId) {
            ...
            case 4: // PDC entity
                ...
                commissionRecipient = this.findItemById(
                    this.tenants,
                    commission.tenantId
                );
                break;
        ...
        const form = this._fb.group({
            ...
            currency: new UntypedFormControl(
                this.findItemById(this.currencies, commission?.currencyId) ??
                    null,
                Validators.required
            ),
            recipientType: new UntypedFormControl(
                this.findItemById(
                    this.commissionRecipientTypeList,
                    commission?.recipientTypeId
                ) ?? null,
                Validators.required
            ),
            recipient: new UntypedFormControl(commissionRecipient ?? null, [
                Validators.required,
                CustomValidators.autocompleteValidator([
                    'clientId',
                    'id',
                    'supplierId',
                ]),
            ...
            frequency: new UntypedFormControl(
                this.findItemById(
                    this.commissionFrequencies,
                    commission?.commissionFrequencyId
                ) ?? null,
                Validators.required
            ),
            oneTimeDate: new UntypedFormControl(
                commission?.oneTimeDate ?? null
            ),
        ...
        this.manageCommissionAutocomplete(
            this.salesMainDataForm.commissions.length - 1
        );
    }

    manageCommissionAutocomplete(commissionIndex: number) {
        let arrayControl =
            this.salesMainDataForm.commissions.at(commissionIndex);
        arrayControl!
            .get('recipient')!
            .valueChanges.pipe(
                takeUntil(this._unsubscribe),
                    ...
                    switch (arrayControl.value.recipientType.id) {
                        ...
                        case 2: // Consultant
                            if (value) {
                                ...
                                return this._lookupService.consultants(
                                    toSend.name,
                                    toSend.maxRecordsCount
                                );
                            } else {
                        ...
                        case 1: // Supplier
                            if (value) {
                                ...
                                return this._lookupService.suppliers(
                                    toSend.name,
                                    toSend.maxRecordsCount
                                );
                            } else {
    ...
    cancelEditCommissionRow(index: number) {
        ...
        commissionRow
            .get('commissionType')
            ?.setValue(this.commissionToEdit?.commissionType);
        commissionRow.get('amount')?.setValue(this.commissionToEdit?.amount);
        commissionRow
            .get('currency')
            ?.setValue(this.commissionToEdit?.currency);
        commissionRow
            .get('commissionFrequency')
            ?.setValue(this.commissionToEdit?.commissionFrequency);
        commissionRow
            .get('recipientType')
            ?.setValue(this.commissionToEdit?.recipientType);
        commissionRow
            .get('recipient')
            ?.setValue(this.commissionToEdit?.recipient);
        this.commissionToEdit = {
            id: undefined,
            commissionType: undefined,
            amount: undefined,
    ...
    getWorkflowSalesStepConsultantTermination(consultant: ConsultantResultDto) {
        ...
        this._workflowServiceProxy
            ...
            .subscribe((result) => {
                ...
                this.salesTerminateConsultantForm.terminationTime?.setValue(
                    result?.terminationTime,
                    { emitEvent: false }
                );
                this.salesTerminateConsultantForm.endDate?.setValue(
                    result?.endDate,
                    { emitEvent: false }
                );
                this.salesTerminateConsultantForm.terminationReason?.setValue(
                    result?.terminationReason,
                    { emitEvent: false }
                ); // add findItemById function
                this.salesTerminateConsultantForm.causeOfNonStandardTerminationTime?.setValue(
                    result?.causeOfNonStandardTerminationTime,
                    { emitEvent: false }
                );
                this.salesTerminateConsultantForm.additionalComments?.setValue(
                ...
                this.salesTerminateConsultantForm.finalEvaluationReferencePerson?.setValue(
                    result?.finalEvaluationReferencePerson,
                    { emitEvent: false }
                ); // add findItemById function
                this.salesTerminateConsultantForm.noEvaluation?.setValue(
                    result?.noEvaluation,
                    { emitEvent: false }
                );
                this.salesTerminateConsultantForm.causeOfNoEvaluation?.setValue(
                    result?.causeOfNoEvaluation,
                    { emitEvent: false }
                );
            });
    }
    ...
    saveTerminationConsultantSalesStep(isDraft: boolean) {
        ...
        input.terminationTime =
            this.salesTerminateConsultantForm?.terminationTime?.value;
        input.endDate = this.salesTerminateConsultantForm?.endDate?.value;
        input.terminationReason =
            +this.salesTerminateConsultantForm?.terminationReason?.value;
        input.causeOfNonStandardTerminationTime =
            this.salesTerminateConsultantForm?.causeOfNonStandardTerminationTime?.value;
        input.additionalComments =
            this.salesTerminateConsultantForm?.additionalComments?.value;

        input.finalEvaluationReferencePersonId = !this
            .salesTerminateConsultantForm?.noEvaluation?.value
            ? this.salesTerminateConsultantForm?.finalEvaluationReferencePerson
                  ?.value?.id
            : null;
        input.noEvaluation =
            this.salesTerminateConsultantForm?.noEvaluation?.value;
        input.causeOfNoEvaluation =
            this.salesTerminateConsultantForm.causeOfNoEvaluation?.value;

        this.showMainSpinner();
        if (isDraft) {
            this._workflowServiceProxy
                .terminationConsultantSalesPut(
                    this.workflowId!,
                    this.consultant.id,
                    input
                )
            ...
            this._workflowServiceProxy
                .terminationConsultantSalesComplete(
                    this.workflowId!,
                    this.consultant.id,
                    input
                )
    ...
    terminateConsultant(index: number) {
        let consultantInformation =
            this.consultantsForm.consultantData.at(index).value.consultantName;
        const scrollStrategy = this.overlay.scrollStrategies.reposition();
        const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
            width: '450px',
            minHeight: '180px',
            ...
            panelClass: 'confirmation-modal',
            data: {
                confirmationMessageTitle: `Are you sure you want to terminate consultant ${
        ...
        dialogRef.componentInstance.onConfirmed.subscribe(() => {
            this.terminateConsultantStart(
                consultantInformation?.consultant?.id!
            );
        });
    ...
    getStartChangeOrExtendConsutlantPeriodSales(consultantPeriodId: string) {
        ...
        this._consultantPeriodSerivce
            ...
            .subscribe((result) => {
                ...
                this.salesClientDataForm.clientPrice?.setValue(
                    result.clientRate?.normalRate,
                    { emitEVent: false }
                );
                this.salesClientDataForm.rateUnitTypeId?.setValue(
                    this.findItemById(
                        this.rateUnitTypes,
                        result.clientRate?.rateUnitTypeId
                    ),
                    { emitEVent: false }
                );
                this.salesClientDataForm.clientCurrency?.setValue(
                ...
                this.salesClientDataForm.clientInvoiceCurrency?.setValue(
                    this.findItemById(
                        this.currencies,
                        result.clientRate?.invoiceCurrencyId
                    ),
                    { emitEVent: false }
                );
                if (
                    ...
                    this.salesClientDataForm.clientInvoiceFrequency?.setValue(
                        this.findItemById(
                            this.invoiceFrequencies,
                            result.clientRate?.invoiceFrequencyId
                        ),
                        { emitEVent: false }
                    );
                }
                if (
                    ...
                    this.salesClientDataForm.clientInvoiceFrequency?.setValue(
                        this.findItemById(
                            this.invoicingTimes,
                            result.clientRate?.invoicingTimeId
                        ),
                        { emitEVent: false }
                    );
                }
                this.salesClientDataForm.clientInvoicingDate?.setValue(
                    result.clientRate?.manualDate,
                    { emitEVent: false }
                );
    ...
    saveStartChangeOrExtendConsultantPeriodSales(isDraft: boolean) {
        ...
        if (
            ...
            consultantInput.consultantRate.manualDate =
                consultant.consultantInvoicingManualDate;
    ...
    formatExpirationNotificationsForDisplay(
        ...
        if (data?.length) {
            ...
            return contractExpirationNotificationDisplay?.length
                ? contractExpirationNotificationDisplay.join(', ')
                : '-';
        } else {
            return '-';
        }
    ...
    openInNewTab(clientId: string) {
        const url = this.router.serializeUrl(
            this.router.createUrlTree([
                `/app/clients/${clientId}/rates-and-fees`,
            ])
        );
