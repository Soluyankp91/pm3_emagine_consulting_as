...
export class EnumEffects {
	constructor(private _actions$: Actions, private readonly _enumService: EnumServiceProxy) {}
	...
	loadCommissionRecipientTypes$ = createEffect(() =>
		this._actions$.pipe(
			...
			map((commissionRecipientTypeList) =>
				enumActions.loadCommissionRecipientTypesSuccess({ commissionRecipientTypeList })
			),
			catchError((error) => of(enumActions.enumError({ error })))
		)
	);
	...
	loadConsultantTimeReportingCaps$ = createEffect(() =>
		this._actions$.pipe(
			...
			map((consultantTimeReportingCapList) =>
				enumActions.loadConsultantTimeReportingCapsSuccess({ consultantTimeReportingCapList })
			),
			catchError((error) => of(enumActions.enumError({ error })))
		)
	);
