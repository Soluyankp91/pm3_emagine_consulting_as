...
import {
	EmployeeNotificationServiceProxy,
	EmployeeTenantNotificationItem,
...
export class NotificationComponent extends AppComponentBase implements OnInit {
	...
	ngOnInit(): void {
		this._titleService.setTitle(ERouteTitleType.Notifications);
			...
			.subscribe((result) => {
				this.tenantWithNotifications = result.map((x) => {
					return new EmployeeNotifications(
						x.tenantId!,
						MapTenantNameFromId(x.tenantId!),
						MapFlagFromTenantId(x.tenantId!),
						x.notifications!
					);
				});
			});
	...
	addNotification(notificationId: number, tenantId: number | undefined) {
		this._notificationService
			.addNotification(notificationId, tenantId)
			.pipe(finalize(() => {}))
			.subscribe();
	}
	...
	deleteNotification(notificationId: number, tenantId: number | undefined) {
		this._notificationService
			.removeNotification(notificationId, tenantId)
			.pipe(finalize(() => {}))
			.subscribe();
	}
}
